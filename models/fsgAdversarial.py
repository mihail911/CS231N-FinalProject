...
def download_val_images (num_ex, mean_image,gold_labels,start,end):
    ...
    for j in xrange(start,end):
        ...
        #     continue
        im = allFiles[j]
        rawimTemp, result = prep_image (path + '/' + str(im), mean_image)
        if result is None:
            continue
    ...
    for i in xrange(num_iter):
		...
		origName = ''
                if adv_prob[k,advLabel] >= 0.5:
                    imName = '/mnt/advResults/high_{0}_{1:.2f}_{2}_{3:.2f}.png'.format(str(classes[trueLabel][0:30]),true_prob[k,trueLabel],
                                                                           str(classes[advLabel][0:30]),adv_prob[k,advLabel])
		    origName = '/mnt/advResults/orig_high_{0}_{1:.2f}_{2}_{3:.2f}.png'.format(str(classes[trueLabel][0:30]),true_prob[k,trueLabel],
                                                                           str(classes[advLabel][0:30]),adv_prob[k,advLabel])
                else:
                    imName = '/mnt/advResults/low_{0}_{1:.2f}_{2}_{3:.2f}.png'.format(str(classes[trueLabel][0:30]),true_prob[k,trueLabel],
                                                                           str(classes[advLabel][0:30]),adv_prob[k,advLabel])
		    origName = '/mnt/advResults/orig_low_{0}_{1:.2f}_{2}_{3:.2f}.png'.format(str(classes[trueLabel][0:30]),true_prob[k,trueLabel],
                                                                           str(classes[advLabel][0:30]),adv_prob[k,advLabel])
                scipy.misc.imsave(imName, final[curCount,:,:,:].transpose(1,2,0).astype('uint8'))
		scipy.misc.imsave(origName, newim[curCount,:,:,:].transpose(1,2,0).astype('uint8'))
                advCount += 1
            else:
                sameAdv.append(adv_prob[k,advLabel])
                sameTrue.append(true_prob[k,trueLabel])
                imName = '/mnt/advResults/nonAdvImage_{0}_{1:.2f}_{2}_{3:.2f}.png'.format(str(classes[trueLabel][0:30]),true_prob[k,trueLabel],
                                                                           str(classes[advLabel][0:30]),adv_prob[k,advLabel])
                scipy.misc.imsave(imName, final[curCount,:,:,:].transpose(1,2,0).astype('uint8'))
                nonAdvCount += 1
